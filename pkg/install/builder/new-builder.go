...
func (c *ArtifactBuilder2) BuildAndWrite(installation profilesv1.ProfileInstallation, artifacts []artifact.Artifact2, repoLocation map[string]string) error {
	fmt.Println("starting building")
	// rootDir := c.RootDir
	// gitRepoName := c.GitRepositoryName
	// gitRepoNamespace := c.GitRepositoryNamespace

	for _, a := range artifacts {
		var deps []artifact.Artifact2
...
func (c *ArtifactBuilder2) writeKustomizeArtifact(installation profilesv1.ProfileInstallation, a artifact.Artifact2, deps []artifact.Artifact2, repoLocations map[string]string) error {
	...
	artifactDir := filepath.Join(c.RootDir, "artifacts", a.NestedProfileDir, a.Name)
	...
	return c.writeResourceWithName(c.makeKustomization(a, filepath.Join(artifactDir, a.Kustomize.Path), installation, a.ProfileName, deps), filepath.Join(artifactDir, "kustomize-flux.yaml"))
}

func (c *ArtifactBuilder2) writeChartArtifact(installation profilesv1.ProfileInstallation, a artifact.Artifact2, deps []artifact.Artifact2, repoLocations map[string]string) error {
	//TODO validate chart
	artifactDir := filepath.Join(c.RootDir, "artifacts", a.NestedProfileDir, a.Name)
	...
	return c.writeResourceWithName(c.makeKustomizeHelmReleaseWrapper(a, installation, a.ProfileName, helmChartDir, deps), filepath.Join(artifactDir, "kustomize-flux.yaml"))
}
...
func (c *ArtifactBuilder2) copyArtifacts(a artifact.Artifact2, subDir, destDir string, repoLocations map[string]string) error {
	tmp, ok := repoLocations[a.RepoKey]
	if !ok {
		return fmt.Errorf("tried to copy artifacts for %q, but repo is not in map %v", a.Name, repoLocations)
	}

	srcDir := filepath.Join(tmp, a.ProfileName, subDir)
...
func (c *ArtifactBuilder2) makeKustomizeHelmReleaseWrapper(artifact artifact.Artifact2, installation profilesv1.ProfileInstallation, definitionName, path string, dependencies []artifact.Artifact2) *kustomizev1.Kustomization {
	wrapper := c.makeKustomization(artifact, path, installation, definitionName, dependencies)
...
func (c *ArtifactBuilder2) makeKustomization(
	...
	dependencies []artifact.Artifact2) *kustomizev1.Kustomization {
...
func containsArtifact(list []artifact.Artifact2, name string) (artifact.Artifact2, bool) {
	for _, a := range list {
		if a.Name == name {
			return a, true
		}
	}
	return artifact.Artifact2{}, false
}
